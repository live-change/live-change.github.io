**@live-change/framework v0.9.201**

***

# @live-change/framework v0.9.201

## Classes

- [ActionDefinition](classes/ActionDefinition.md)
- [EventDefinition](classes/EventDefinition.md)
- [ForeignIndexDefinition](classes/ForeignIndexDefinition.md)
- [ForeignModelDefinition](classes/ForeignModelDefinition.md)
- [IndexDefinition](classes/IndexDefinition.md)
- [ModelDefinition](classes/ModelDefinition.md)
- [PropertyDefinition](classes/PropertyDefinition.md)
- [QueryDefinition](classes/QueryDefinition.md)
- [ServiceDefinition](classes/ServiceDefinition.md)
- [TriggerDefinition](classes/TriggerDefinition.md)
- [ViewDefinition](classes/ViewDefinition.md)

## Interfaces

- [ActionContext](interfaces/ActionContext.md)
- [ActionDefinitionSpecification](interfaces/ActionDefinitionSpecification.md)
- [ClientContext](interfaces/ClientContext.md)
- [ContextBase](interfaces/ContextBase.md)
- [EventDefinitionSpecification](interfaces/EventDefinitionSpecification.md)
- [IndexDefinitionSpecification](interfaces/IndexDefinitionSpecification.md)
- [ModelDefinitionSpecification](interfaces/ModelDefinitionSpecification.md)
- [ModelIndexDefinitionSpecification](interfaces/ModelIndexDefinitionSpecification.md)
- [ModelPropertyDefinitionSpecification](interfaces/ModelPropertyDefinitionSpecification.md)
- [PropertyDefinitionSpecification](interfaces/PropertyDefinitionSpecification.md)
- [QueryDefinitionSpecification](interfaces/QueryDefinitionSpecification.md)
- [ServiceDefinitionSpecification](interfaces/ServiceDefinitionSpecification.md)
- [TriggerContext](interfaces/TriggerContext.md)
- [TriggerDefinitionSpecification](interfaces/TriggerDefinitionSpecification.md)
- [TriggerServiceSettings](interfaces/TriggerServiceSettings.md)
- [TriggerSettings](interfaces/TriggerSettings.md)
- [ValidationSpecificationObject](interfaces/ValidationSpecificationObject.md)
- [ViewContext](interfaces/ViewContext.md)
- [ViewDefinitionSpecificationBase](interfaces/ViewDefinitionSpecificationBase.md)
- [ViewDefinitionSpecificationDaoPath](interfaces/ViewDefinitionSpecificationDaoPath.md)
- [ViewDefinitionSpecificationFetch](interfaces/ViewDefinitionSpecificationFetch.md)
- [ViewDefinitionSpecificationObservable](interfaces/ViewDefinitionSpecificationObservable.md)

## Type Aliases

- [AccessFunction](type-aliases/AccessFunction.md)
- [AccessSpecification](type-aliases/AccessSpecification.md)
- [ActionParameters](type-aliases/ActionParameters.md)
- [EventParameters](type-aliases/EventParameters.md)
- [QueryParameters](type-aliases/QueryParameters.md)
- [TriggerParameters](type-aliases/TriggerParameters.md)
- [ValidationSpecification](type-aliases/ValidationSpecification.md)
- [ViewDefinitionSpecification](type-aliases/ViewDefinitionSpecification.md)

## Variables

- [unknown](variables/unknown.md)

## References

### EventDefinitionSpecificationAC

Renames and re-exports [EventDefinitionSpecification](interfaces/EventDefinitionSpecification.md)
